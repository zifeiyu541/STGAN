...
class MyWindow:
    ...
    def __init__(self):
        ...
        for cat in paths:
            ...
            getattr(self, 'lbl_%s_rec'%cat).pack(side=t.LEFT, padx=3)
            for i in range(num_att):
                ...
                getattr(self, 'lbl_%s_%s'%(cat, atts[i])).pack(side=t.LEFT, padx=3)
            getattr(self, 'can_%s'%(cat)).pack(fill=t.X)
        
        row_att = t.Canvas(self.can)
        self.Label_rec = t.Button(row_att, text='label_rec', command=self.save_rec, width=16)
