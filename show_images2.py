...
class MyWindow:
    ...
    def __init__(self):
        ...
        for cat in paths:
            ...
            setattr(self, 'lbl_%s_%s' % (cat, atts[0]),
                        t.Label(getattr(self, 'can_%s' % (cat)), image=getattr(self, 'img_%s_%s' % (cat, atts[0]))))

        row_att = t.Canvas(self.can)
        self.Label_rec = t.Button(row_att, text='Original picture', command=self.save_rec, width=20)
        ...
        for att in atts:
            ...
            setattr(self, 'Label_%s' % att, t.Button(row_att, text=getattr(self, 'label_%s' % att),
                                                     command=getattr(self, 'save_%s' % att), width=20))
        ...
        for name, num in site:
            if num > 6:
                break
            radio_button = t.Radiobutton(atts_can1, text=name, variable=self.v, value=num,
                                         indicatoron=False, command=self.refresh)
            radio_button.pack(side=t.LEFT)
        atts_can1.pack(anchor=t.W, side=t.TOP)
        atts_can2 = t.Canvas(self.row0)
        for name, num in site:
            if num <= 6:
                continue
            radio_button = t.Radiobutton(atts_can2, text=name, variable=self.v, value=num,
                                         indicatoron=False, command=self.refresh)
            radio_button.pack(side=t.LEFT)
        atts_can2.pack(anchor=t.W, side=t.TOP)
        self.row0.pack(fill=t.X, side=t.TOP, anchor=t.CENTER)
        # for i in range(num_att):
        #     setattr(self, atts[i], t.BooleanVar(value=False))

        for cat in paths:
            getattr(self, 'lbl_%s_rec'%cat).pack(side=t.LEFT, padx=20)

            # setattr(self, atts[self.v.get()], t.BooleanVar(value=True))
            getattr(self, 'lbl_%s_%s'%(cat, atts[0])).pack(side=t.LEFT, padx=20)
            getattr(self, 'can_%s'%(cat)).pack(fill=t.X)
        
        self.Label_rec.pack(side=t.LEFT)

        getattr(self, 'Label_%s'%atts[0]).pack(side=t.LEFT)
        row_att.pack(fill=t.X)
        self.can.pack(fill=t.BOTH)
        
        window_width = (128+10)*(4)
        ...
        for cat in rec_paths:
            try:
                ...
                try:
                    ...
                    ts.generate_single(experiment_name="128", img=[int(num)])
                    return self.process_img(num)
...
#        print(imgs)
        ...
        for cat in paths:
            ...
            # if not getattr(self, atts[self.v.get()]).get():
            #     setattr(self, 'img_%s_%s'%(cat, atts[self.v.get()]), self.grayTkimg)
            #     getattr(self, 'lbl_%s_%s'%(cat, atts[self.v.get()])).config(image=self.grayTkimg)
            #     continue
            setattr(self, 'img_%s_%s'%(cat, atts[self.v.get()]), ImageTk.PhotoImage(imgs[cat][self.v.get()]))
            getattr(self, 'lbl_%s_%s'%(cat, atts[0]))\
                .config(image=getattr(self, 'img_%s_%s'%(cat, atts[self.v.get()])))
            # getattr(self, 'lbl_%s_%s' % (cat, atts[self.v.get()])).pack(side=t.LEFT, padx=3)
#                getattr(self, 'lbl_%s_%s'%(cat, atts[i])).image = ImageTk.PhotoImage(imgs[cat][i])

        if attr[int(self.num.get())-1][att_dict[atts[self.v.get()]]] == '1':
            setattr(self, 'label_%s'%atts[self.v.get()], '---'+atts_show[atts[self.v.get()]]+'---')
        else:
            setattr(self, 'label_%s'%atts[self.v.get()], '+++'+atts_show[atts[self.v.get()]]+'+++')
        getattr(self, 'Label_%s'%atts[0]).config(text=getattr(self, 'label_%s'%atts[self.v.get()]))
